...
{
    class ConvolutionForward_Method : FunctionBase
    {
        ...
        protected override void CpuFunction(ComputeVariable variable)
        {
            ...
            var output_temporary = variable[1].Instance.Array.Data;
            var output = variable[2].Instance.Array.Data;
            var kn = variable[3].Instance.Array.Data;
            var bias = variable[4].Instance.Array.Data;

            var inputparam = variable[0].Instance;
            var outputparam = variable[1].Instance;
            var kernelparam = variable[3].Instance;
        ...
        protected override void GpuFunction(ComputeVariable variable)
        {
            ...
            using (Cloo.ComputeBuffer<Real> _output_temporary = ConvertBuffer(variable[1].Instance))
            using (Cloo.ComputeBuffer<Real> _output = ConvertBuffer(variable[2].Instance))
            using (Cloo.ComputeBuffer<Real> _kernel = ConvertBuffer(variable[3].Instance))
            using (Cloo.ComputeBuffer<Real> _bias = ConvertBuffer(variable[4].Instance))
            {
                SetParameter(_input);
                SetParameter(_output_temporary);
                ...
                SetParameter(activation, ValueMode.INT);
                SetParameter(alpha, ValueMode.FLOAT);
                Execute(batsize, outch, oasize);
                ReadBuffer(_output_temporary, ref variable[1].Instance.Array.Data);
                ReadBuffer(_output, ref variable[2].Instance.Array.Data);
            }
